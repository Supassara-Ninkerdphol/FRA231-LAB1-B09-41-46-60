...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) encoder_M->Timing.taskTime0;
  encoder_step();
...
int main(int argc, char **argv)
{
  ...
  errorCode = extmodeInit(encoder_M->extModeInfo, &rtmGetTFinal(encoder_M));
  if (errorCode != EXTMODE_SUCCESS) {
